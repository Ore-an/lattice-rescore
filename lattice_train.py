...
class Cache(dict):
    ...
    """
    __slots__ = 'model', 'dict', 'type', 'sp', 'h', 'cache_locality', 'acronyms', 'device', 'train', 'store_grad_cpu'
    ...
    def __init__(self, model, dictionary, feat=None, model_type='rnnlm',
                 sp=None, cache_locality=9, acronyms={}, device='cpu', train=False, mapping=(), store_grad_cpu=False, **kwargs):
        ...
        for expnode in node.expnodes:
            for earc in expnode.entries:
                if earc.src.sym != lattice.SOS: # SOS is initialized, not a model output so no grad
                    ...
                    ac_tensor = -torch.clamp(torch.tensor(earc.ascr, device=device), 0, None) # ascr is nll
